...
class DDQNGameModel(BaseGameModel):
    ...
    def __init__(self, mode_name, input_shape, action_space, logger_path, model_path):
        BaseGameModel.__init__(self, mode_name,
                               logger_path,
                               input_shape,
                               action_space)
...
class DDQNSolver(DDQNGameModel):
    ...
    def __init__(self, input_shape, action_space):
        testing_model_path = "./output/neural_nets/ddqn/testing/model.h5"
        assert os.path.exists(os.path.dirname(testing_model_path)), "No testing model in: " + str(testing_model_path)
        DDQNGameModel.__init__(self, "DDQN testing", input_shape, action_space, 
                                "./output/logs/ddqn/testing/" + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "/", 
                                testing_model_path)
...
class DDQNTrainer(DDQNGameModel):
    ...
    def __init__(self, input_shape, action_space):
        DDQNGameModel.__init__(self, "DDQN training", input_shape, action_space, 
                                "./output/logs/ddqn/training/" + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "/",
                                "./output/neural_nets/ddqn/" + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "/model.h5")
